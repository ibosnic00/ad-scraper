...
class NjuskaloCrawler():
    ...
    def _initializeStartClicks(self, page):
        ...
        try:
            page.click("#didomi-notice-agree-button", timeout = random.uniform(0.5,1.5) * 500)
        except:
            print("Cookie policy accept button not found")
        time.sleep(random.uniform(1.5,2.5))
        try:
            page.locator('.button-standard .button-standard--alpha .bp-radix__button-standard--full .PrivacyPolicyNotice-close .PrivacyPolicyNotice-close--asButton').click(timeout = random.uniform(0.5,1.5) *500)
        except:
